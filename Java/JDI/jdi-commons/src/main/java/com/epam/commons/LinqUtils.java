...
public final class LinqUtils {
    ...
    public static <T> boolean any(T[] array, JFuncTREx<T, Boolean> func) {
        return any(asList(array), func);
    }
    public static <T> T single(Collection<T> list, JFuncTREx<T, Boolean> func) {
        if (list == null || list.size() == 0)
            return null;
        T found = null;
        try {
    ...
    public static <T> T single(T[] array, JFuncTREx<T, Boolean> func) {
