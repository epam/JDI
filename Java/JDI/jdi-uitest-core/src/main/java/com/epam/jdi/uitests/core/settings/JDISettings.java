...
public abstract class JDISettings {
  ...
  public static synchronized void initFromProperties() throws IOException {
    ...
    fillAction(
        p -> USE_CACHE = p.toLowerCase().equals("true") || p.toLowerCase().equals("1"), "cache");
    fillAction(
        p -> isDemoMode = p.toLowerCase().equals("true") || p.toLowerCase().equals("1"),
        "demo.mode");
    fillAction(
        p ->
            highlightSettings.setBgColor(
                HighlightSettings.parseColor(p, CssColors.MEDIUM_SLATE_BLUE)),
        "demo.color.background");
    fillAction(
        p -> highlightSettings.setFrameColor(HighlightSettings.parseColor(p, CssColors.DIM_GRAY)), "demo.color.frame");
    fillAction(p -> highlightSettings.setFontColor(HighlightSettings.parseColor(p, CssColors.BLACK)), "demo.color.text");
