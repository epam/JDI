...
class Columns(TableLine):
    ...
    def get_first_line(self):
        return self.table.rows.get_line_action(1)

    def get_column(self, val):
        try:
            row_count = self.table.rows.get_count()
            web_column = Time.wait_result_by_conditions(lambda: self.get_line_action(val),
                                                        lambda els: len(els) == row_count)
            res = list()
            ...
            if isinstance(val, int):
                ...
                if self.get_count() < 0 or self.get_count() < val or val <= 0:
                    raise Exception("Can't Get Column '{0}'. [num] > RowsCount({1}).".format(val, self.get_count()))

                for i in range(row_count):
                    res.append(tuple([self.table.rows.get_headers()[i],
                                      self.table.cell(web_column[i], Column(val), Row(i + 1))]))

            elif isinstance(val, str):
                headers = self.table.rows.get_headers()
                for i in range(row_count):
                    res.append(tuple([self.table.rows.get_headers()[i],
                                      self.table.cell(web_column[i], Column(val), Row(headers[i]))]))
            else:
                raise ValueError("Value for 'get_column' should string or int")
            return res
